...
成功安装Hexo后，运行下面的命令，会在指定文件夹中新建必须的文件。
```bash
hexo init 文件夹名
cd 文件夹名
npm install
...
执行
```bash
git clone https://gitee.com/immyw/hexo-theme-butterfly themes/butterfly
```
### 启用主题
...
:::
## 配置
在hexo根目录下新建_config.butterfly.yml
写进如下内容
```yaml
...
```
::: success
然后进入_config.butterfly.yml里自行配置
:::
